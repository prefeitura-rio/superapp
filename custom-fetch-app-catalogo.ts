...
const getUrl = (contextUrl: string): string => {
  const baseUrl = process.env.NEXT_PUBLIC_BASE_API_URL_APP_CATALOGO

  if (!baseUrl) {
    throw new Error(
      'NEXT_PUBLIC_BASE_API_URL_APP_CATALOGO environment variable is not set.'
    )
  }
...
const getHeaders = async (headers?: HeadersInit): Promise<HeadersInit> => {
  ...
  if (!access_token) {
    console.warn('[CUSTOM_FETCH_APP_CATALOGO] No access token found in cookies')
  }
...
export const customFetchAppCatalogo = async <T>(
  url: string,
  options: RequestInit
): Promise<T> => {
