...
const getUrl = (contextUrl: string): string => {
  const baseUrl = process.env.BASE_API_URL_DIVIDA_ATIVA
...
const getHeaders = async (headers?: HeadersInit): Promise<HeadersInit> => {
  ...
  if (!access_token) {
    console.warn('[CUSTOM_FETCH_DIVIDA_ATIVA] No access token found in cookies')
  }
  ...
  return {
    ...
    // Don't override Content-Type if it's already set (multipart uploads set their own)
    ...(hasContentType ? {} : { 'Content-Type': 'application/json' }),
  }
}

export const customFetchDividaAtiva = async <T>(
  url: string,
  options: RequestInit
): Promise<T> => {
