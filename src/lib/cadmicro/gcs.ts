import {
  ...
  CADMICRO_PATH_PREFIX,
  type CadmicroFileKind,
  isCadmicroFileKind,
  normalizeCpfDigits,
...
export function buildCadmicroObjectPath(
  cpfDigits: string,
  kind: CadmicroFileKind,
  contentType: string
): string {
  const ext = CADMICRO_ALLOWED_CONTENT_TYPES[contentType]
  const uuid = crypto.randomUUID()
  return `${CADMICRO_PATH_PREFIX}/${cpfDigits}/${kind}/${uuid}.${ext}`
}
...
export function parseCadmicroObjectUrl(
  objectUrl: string,
  bucketName: string
  ...
  // GCS object path without bucket: mobilidade/<cpf>/<kind>/<uuid>.<ext>
  const objectPath = decodeURIComponent(
    parsed.pathname.slice(bucketPrefix.length)
  )
  if (!objectPath.startsWith(`${CADMICRO_PATH_PREFIX}/`)) return null

  const segments = objectPath.split('/')
  if (segments.length !== 4) return null
  if (segments[0] !== CADMICRO_PATH_PREFIX) return null
  if (!/^\d{11}$/.test(segments[1])) return null
  if (!isCadmicroFileKind(segments[2])) return null
