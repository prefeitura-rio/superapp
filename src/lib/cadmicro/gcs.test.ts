...
const VALID_OBJECT_PATH = `mobilidade/${OWNER_CPF}/vehicle/a0f28c74-b297-482b-b6fd-ed43c58aa6c8.png`
const VALID_URL = `https://storage.googleapis.com/${BUCKET}/${VALID_OBJECT_PATH}`
const SERIAL_URL = `https://storage.googleapis.com/${BUCKET}/mobilidade/${OWNER_CPF}/serial/b1f28c74-b297-482b-b6fd-ed43c58aa6c8.pdf`
const OTHER_URL = `https://storage.googleapis.com/${BUCKET}/mobilidade/${OWNER_CPF}/invoice/c2f28c74-b297-482b-b6fd-ed43c58aa6c8.png`

describe('parseCadmicroObjectUrl', () => {
  it('parses a valid CadMicro object URL (strips bucket from path)', () => {
    expect(parseCadmicroObjectUrl(VALID_URL, BUCKET)).toEqual({
  ...
  it('returns null when bucket does not match', () => {
    expect(parseCadmicroObjectUrl(VALID_URL, 'other-bucket')).toBeNull()
  })

  it('returns null when path is not under mobilidade/', () => {
    const url = `https://storage.googleapis.com/${BUCKET}/superapp/images/courses/a0f28c74-b297-482b-b6fd-ed43c58aa6c8.png`
    expect(parseCadmicroObjectUrl(url, BUCKET)).toBeNull()
  })
  ...
  it('returns null for malformed URL', () => {
    expect(parseCadmicroObjectUrl('not-a-url', BUCKET)).toBeNull()
  })
})
