...
function isValidCadmicroPhotoUrl(url: string): boolean {
...
function gcsPhotoUrlSchema(requiredMessage: string) {
  return z.string().min(1, requiredMessage).refine(isValidCadmicroPhotoUrl, {
...
function refineSerialPhotosFields(
  ...
  if (data.has_invoice === true) {
    ...
    } else if (!isValidCadmicroPhotoUrl(data.invoice_photo_url)) {
      ctx.addIssue({
        code: 'custom',
        path: ['invoice_photo_url'],
        message: 'Envie um arquivo válido (upload para o armazenamento)',
