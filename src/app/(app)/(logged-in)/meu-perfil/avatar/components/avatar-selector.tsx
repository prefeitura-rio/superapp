...
export function AvatarSelector({ avatars, currentAvatarId }: AvatarSelectorProps) {
  ...
  const validAvatars: ValidAvatar[] = avatars
    .filter((avatar): avatar is ValidAvatar => 
  ...
  return (
    <div className="px-4">
      <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 mt-6 justify-items-center place-items-center">
        {validAvatars.map((avatar) => {
          ...
          return (
            <div
              key={avatar.id}
              className={`w-full md:max-w-24 xl:max-w-32 aspect-square rounded-full border-2 bg-transparent flex items-center justify-center cursor-pointer transition-colors overflow-hidden group relative ${
                isSelected 
                  ? 'border-card-2' 
      ...
      <div className="mt-10 md:mt-16 xl:mt-20">
        ...
        >
          {isLoading ? "Salvando..." : isCurrentAvatarSelected ? "Voltar" : "Salvar"}
        </CustomButton>
      </div>
    </div>
  );
