...
import type { ChangeScheduleFormData, NearbyUnit } from '../../types'

interface SelectNewUnitSlideProps {
...
export const SelectNewUnitSlide = ({
  nearbyUnits,
  form,
  fieldName,
  currentEnrollment,
}: SelectNewUnitSlideProps) => {
  const {
    formState: { errors },
    setValue,
    watch,
  ...
  // Get current enrolled unit ID from enrollment
        ...
        >
          ...
          >
            {nearbyUnits.map((unit, index) => {
              ...
              const isCurrentUnit = unit.id === currentEnrolledUnitId
              return (
                <label
                  key={unit.id}
                  htmlFor={unit.id}
