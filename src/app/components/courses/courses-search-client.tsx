...
interface CoursesSearchClientProps {
  initialCategoryFilters: CategoryFilter[]
  initialCourses?: ModelsCurso[]
}
...
export default function CoursesSearchClient({
  ...
  const [initialCoursesState, setInitialCoursesState] =
    ...
    async function loadInitialCourses() {
      ...
      if (!hasQuery && !hasFilters) {
        if (initialCourses.length > 0) {
          ...
          setInitialCoursesState(transformedCourses)
        } else {
          try {
  ...
  const saveSearchToHistory = (searchQuery: string) => {
    ...
    try {
      ...
      const prev = savedHistory ? JSON.parse(savedHistory) : []
    ...
    <main className="min-h-lvh max-w-4xl mx-auto text-foreground pb-16">
      ...
      {!showResults && (
        <div className="px-4 pb-30 mt-6 space-y-6">
          ...
          {topCourses.length === 0 && initialCoursesState.length > 0 && (
            <div>
              <h2 className="text-base font-medium text-foreground mb-4">
                Cursos mais procurados
              </h2>
              <div>
                {initialCoursesState.map(course => (
                  <Link
                    key={course.id}
                    href={`/servicos/cursos/${createCourseSlug(
                      course.id?.toString() || '',
