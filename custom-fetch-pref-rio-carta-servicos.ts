...
const getUrl = (contextUrl: string): string => {
  const baseUrl = process.env.BASE_API_URL_PREF_RIO_CARTA_SERVICOS

  if (!baseUrl) {
    throw new Error(
      'BASE_API_URL_PREF_RIO_CARTA_SERVICOS environment variable is not set.'
    )
  }
...
export const customFetchPrefRioCartaServicos = async <T>(
  url: string,
  options: RequestInit
): Promise<T> => {
