...
const getUrl = (contextUrl: string): string => {
  const baseUrl = process.env.BASE_API_URL_PREF_RIO_CIDADAO

  if (!baseUrl) {
    throw new Error(
      'BASE_API_URL_PREF_RIO_CIDADAO environment variable is not set.'
    )
  }
...
const getHeaders = async (headers?: HeadersInit): Promise<HeadersInit> => {
  ...
  if (!access_token) {
    console.warn(
      '[CUSTOM_FETCH_PREF_RIO_CIDADAO] No access token found in cookies'
    )
  }
...
export const customFetchPrefRioCidadao = async <T>(
  url: string,
  options: RequestInit
): Promise<T> => {
