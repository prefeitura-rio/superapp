...
const getUrl = (contextUrl: string): string => {
  const baseUrl = process.env.BASE_API_URL_APP_BUSCA_SEARCH

  if (!baseUrl) {
    throw new Error(
      'BASE_API_URL_APP_BUSCA_SEARCH environment variable is not set.'
    )
  }
...
const getHeaders = async (headers?: HeadersInit): Promise<HeadersInit> => {
  ...
  if (!access_token) {
    console.warn('[CUSTOM_FETCH_BUSCA] No access token found in cookies')
  }
...
export const customFetchBuscaSearch = async <T>(
  url: string,
  options: RequestInit
): Promise<T> => {
